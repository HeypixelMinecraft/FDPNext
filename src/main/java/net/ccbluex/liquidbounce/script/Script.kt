...
class Script(private val scriptFile: File) : MinecraftInstance() {
    ...
    init {
        ...
        scriptEngine.put("moduleManager", FDPNext.moduleManager)
        scriptEngine.put("commandManager", FDPNext.commandManager)
        scriptEngine.put("scriptManager", FDPNext.scriptManager)
    ...
    fun registerModule(moduleObject: JSObject, callback: JSObject) {
        ...
        FDPNext.moduleManager.registerModule(module)
    ...
    fun registerCommand(commandObject: JSObject, callback: JSObject) {
        ...
        FDPNext.commandManager.registerCommand(command)
        registeredCommands += command
        callback.call(commandObject, command)
    }

    fun regAnyThing() {
        registeredModules.forEach { FDPNext.moduleManager.registerModule(it) }
        registeredCommands.forEach { FDPNext.commandManager.registerCommand(it) }
    }
    ...
    fun supportLegacyScripts() {
        if (!scriptText.lines().first().contains("api_version=2")) {
            ...
            val legacyScript =
                FDPNext::class.java.getResource("/assets/minecraft/FDPNext/scriptapi/legacy.js")?.readText()
    ...
    fun onDisable() {
        ...
        registeredModules.forEach { FDPNext.moduleManager.unregisterModule(it) }
        registeredCommands.forEach { FDPNext.commandManager.unregisterCommand(it) }
    ...
    fun import(scriptFile: String) {
        scriptEngine.eval(File(FDPNext.scriptManager.scriptsFolder, scriptFile).readText())
    }
