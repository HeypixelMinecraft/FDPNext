...
public abstract class MixinMinecraft {
    ...
    private void displayGuiScreen(CallbackInfo callbackInfo) {
        if (currentScreen instanceof net.minecraft.client.gui.GuiMainMenu || (currentScreen != null && currentScreen.getClass().getName().startsWith("net.labymod") && currentScreen.getClass().getSimpleName().equals("ModGuiMainMenu"))) {
            currentScreen = FDPNext.mainMenu;
        ...
        FDPNext.eventManager.callEvent(new ScreenEvent(currentScreen));
    }
    ...
    private void onTick(final CallbackInfo callbackInfo) {
        FDPNext.eventManager.callEvent(new TickEvent());
    }
    ...
    private void onKey(CallbackInfo callbackInfo) {
        try {
            if (Keyboard.getEventKeyState() && (currentScreen == null || (SoundModule.INSTANCE.getToggleIgnoreScreenValue().get() && this.currentScreen instanceof GuiContainer)))
                FDPNext.eventManager.callEvent(new KeyEvent(Keyboard.getEventKey() == 0 ? Keyboard.getEventCharacter() + 256 : Keyboard.getEventKey()));
        } catch (Exception e) {
            //e.printStackTrace();
        }
    }
    ...
    private void onClickBlock(CallbackInfo callbackInfo) {
        if (this.leftClickCounter == 0 && theWorld.getBlockState(objectMouseOver.getBlockPos()).getBlock().getMaterial() != Material.air) {
            FDPNext.eventManager.callEvent(new ClickBlockEvent(objectMouseOver.getBlockPos(), this.objectMouseOver.sideHit));
        }
    }

    @Inject(method = "shutdown", at = @At("HEAD"))
    private void shutdown(CallbackInfo callbackInfo) {
        FDPNext.INSTANCE.stopClient();
    }

    @Inject(method = "clickMouse", at = @At("HEAD"))
    private void clickMouse(CallbackInfo callbackInfo) {
        CPSCounter.registerClick(CPSCounter.MouseButton.LEFT);
        if (FDPNext.moduleManager.getModule(AutoClicker.class).getState())
            leftClickCounter = 0;
    }
    ...
    private void rightClickMouse(final CallbackInfo callbackInfo) {
        ...
        final FastPlace fastPlace = FDPNext.moduleManager.getModule(FastPlace.class);
    ...
    private void loadWorld(WorldClient p_loadWorld_1_, String p_loadWorld_2_, final CallbackInfo callbackInfo) {
        FDPNext.eventManager.callEvent(new WorldEvent(p_loadWorld_1_));
    }
    ...
    public void getRenderViewEntity(CallbackInfoReturnable<Entity> cir) {
        if (RotationUtils.targetRotation != null && thePlayer != null) {
            final Rotations rotations = FDPNext.moduleManager.getModule(Rotations.class);
    ...
    private void sendClickBlockToController(boolean leftClick) {
        ...
        if (this.leftClickCounter <= 0 && (!this.thePlayer.isUsingItem() || FDPNext.moduleManager.getModule(MultiActions.class).getState())) {
            if (leftClick && this.objectMouseOver != null && this.objectMouseOver.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK) {
                BlockPos blockPos = this.objectMouseOver.getBlockPos();

                if (this.leftClickCounter == 0)
                    FDPNext.eventManager.callEvent(new ClickBlockEvent(blockPos, this.objectMouseOver.sideHit));
    ...
    private void setWindowIcon(CallbackInfo callbackInfo) {
        try {
            if (Util.getOSType() != Util.EnumOS.OSX) {
                BufferedImage image = ImageIO.read(this.getClass().getResourceAsStream("/assets/minecraft/FDPNext/misc/icon.png"));
