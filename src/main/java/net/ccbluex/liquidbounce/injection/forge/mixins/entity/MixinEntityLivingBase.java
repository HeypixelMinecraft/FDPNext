...
public abstract class MixinEntityLivingBase extends MixinEntity {
    ...
    protected void jump() {
        ...
        FDPNext.eventManager.callEvent(jumpEvent);
    ...
    private void headLiving(CallbackInfo callbackInfo) {
        if (FDPNext.moduleManager.getModule(NoJumpDelay.class).getState())
            jumpTicks = 0;
    }

    @Inject(method = "onLivingUpdate", at = @At(value = "FIELD", target = "Lnet/minecraft/entity/EntityLivingBase;isJumping:Z", ordinal = 1))
    private void onJumpSection(CallbackInfo callbackInfo) {
        final Jesus jesus = FDPNext.moduleManager.getModule(Jesus.class);
    ...
    private void isPotionActive(Potion p_isPotionActive_1_, final CallbackInfoReturnable<Boolean> callbackInfoReturnable) {
        final AntiBlind antiBlind = FDPNext.moduleManager.getModule(AntiBlind.class);
