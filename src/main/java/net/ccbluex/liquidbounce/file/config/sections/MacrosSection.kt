...
class MacrosSection : ConfigSection("macros") {
    override fun load(json: JsonObject) {
        FDPNext.macroManager.macros.clear()

        val jsonArray = json.getAsJsonArray("macros") ?: return

        for (jsonElement in jsonArray) {
            val macroJson = jsonElement.asJsonObject
            FDPNext.macroManager.macros.add(Macro(macroJson.get("key").asInt, macroJson.get("command").asString))
        }
    }
    ...
    override fun save(): JsonObject {
        ...
        for (macro in FDPNext.macroManager.macros) {
