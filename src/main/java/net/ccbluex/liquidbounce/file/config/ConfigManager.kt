...
class ConfigManager {
    private val configSetFile = File(FDPNext.fileManager.dir, "config-settings.json")

    private val sections = mutableListOf<ConfigSection>()

    var nowConfig = "default"
    private var nowConfigInFile = "default"
    var configFile = File(FDPNext.fileManager.configsDir, "$nowConfig.json")
    var needSave = false
    ...
    fun save(saveConfigSet: Boolean = nowConfigInFile != nowConfig, forceSave: Boolean = false) {
        if (FDPNext.isLoadingConfig && !forceSave) {
            return
        }
    ...
    fun loadLegacySupport() {
        if (FDPNext.fileManager.loadLegacy()) {
            if (File(FDPNext.fileManager.configsDir, "$nowConfig.json").exists()) {
                nowConfig = "legacy"
                configFile = File(FDPNext.fileManager.configsDir, "$nowConfig.json")
                save(forceSave = true)
            } else {
            ...
            script.lines().filter { it.isNotEmpty() && !it.startsWith('#') }.forEachIndexed { _, s ->
                ...
                when (args[0]) {
                    ...
                    "targetDead" -> {
                        ...
                        val value = args[2]
                        val module = FDPNext.moduleManager.getModule(moduleName) ?: return@forEachIndexed
        ...
        val oldSettingDir = File(FDPNext.fileManager.dir, "settings")
        if (oldSettingDir.exists()) {
            oldSettingDir.listFiles().forEach {
                ...
                if (!FDPNext.fileManager.legacySettingsDir.exists()) {
                    FDPNext.fileManager.legacySettingsDir.mkdir()
                }

                it.renameTo(File(FDPNext.fileManager.legacySettingsDir, it.name))
            }
