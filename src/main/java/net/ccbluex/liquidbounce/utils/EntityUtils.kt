...
object EntityUtils : MinecraftInstance() {
            ...
            if (invisibleValue.get() || !entity.isInvisible()) {
                if (playerValue.get() && entity is EntityPlayer) {
                    if (canAttackCheck) {
                        ...
                        if (!FDPNext.combatManager.isFocusEntity(entity)) {
                            return false
                        }
    ...
    fun isFriend(entity: Entity): Boolean {
        return entity is EntityPlayer && entity.getName() != null && FDPNext.fileManager.friendsConfig.isFriend(stripColor(entity.getName()))
    }

    fun isFriend(entity: String): Boolean {
        return FDPNext.fileManager.friendsConfig.isFriend(entity)
    }
