    ...
    override fun execute(args: Array<String>) {
        if (args.size == 3) {
            ...
            when (focused.lowercase()) {
                "add" -> {
                    FDPNext.combatManager.focusedPlayerList.add(entity)
                    alert("Successfully added §a${target.lowercase()}§3 into the focus list.")
                    return
                }
                "remove" -> {
                    if (FDPNext.combatManager.focusedPlayerList.contains(entity)) {
                        FDPNext.combatManager.focusedPlayerList.remove(entity)
            ...
            FDPNext.combatManager.focusedPlayerList.clear()
