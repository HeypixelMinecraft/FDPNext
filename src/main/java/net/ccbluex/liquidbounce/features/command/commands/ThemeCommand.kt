...
class ThemeCommand : Command("theme", arrayOf("thememanager", "tm", "themes")) {
        ...
        if (args.size > 1) {
            when {
                args[1].equals("load", ignoreCase = true) -> {
                    if (args.size > 2) {
                        val themeFile = File(FDPNext.fileManager.themesDir, args[2])
                        ...
                        if (themeFile.exists()) {
                            try {
                                ...
                                FDPNext.hud = Config(theme).toHUD()
                                FDPNext.isStarting = false
                                chat("§6Theme applied successfully.")
                                FDPNext.hud.addNotification(Notification("theme", "Updated HUD Theme.", NotifyType.SUCCESS))
                                playEdit()
                            } catch (e: IOException) {
                                e.printStackTrace()
                            }
                ...
                args[1].equals("save", ignoreCase = true) -> {
                    if (args.size > 2) {
                        ...
                        try {
                            ...
                            val settingsTheme = Config(FDPNext.hud).toJson()
                            chat("§9Saving theme...")
    ...
    private fun getLocalThemes(): Array<File>? = FDPNext.fileManager.themesDir.listFiles()
}
