    ...
    override fun execute(args: Array<String>) {
        if (args.size > 1) {
            ...
            when (arg1.lowercase()) {
                "add" -> {
                    if (args.size > 3) {
                        ...
                        if (key != Keyboard.KEY_NONE) {
                            ...
                            FDPNext.macroManager.macros.add(Macro(key, comm))
                ...
                "remove" -> {
                    if (args.size > 2) {
                        if (args[2].startsWith(".")) {
                            FDPNext.macroManager.macros.filter { it.command == StringUtils.toCompleteString(args, 2) }
                        } else {
                            val key = Keyboard.getKeyIndex(args[2].uppercase())
                            FDPNext.macroManager.macros.filter { it.key == key }
                        }.forEach {
                            FDPNext.macroManager.macros.remove(it)
                ...
                "list" -> {
                    alert("Macros:")
                    FDPNext.macroManager.macros.forEach {
