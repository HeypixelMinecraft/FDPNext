...
class HelpCommand : Command("help", emptyArray()) {
    ...
    override fun execute(args: Array<String>) {
        ...
        val maxPageDouble = FDPNext.commandManager.commands.size / 8.0
        ...
        val commands = FDPNext.commandManager.commands.map { it.value }.distinct().sortedBy { it.command }

        var i = 8 * (page - 1)
        while (i < 8 * page && i < commands.size) {
            val command = commands[i]

            ClientUtils.displayChatMessage("§6> §7${FDPNext.commandManager.prefix}${command.command}${if (command.alias.isEmpty()) "" else " §7(§8" + Strings.join(command.alias, "§7, §8") + "§7)"}")
            i++
        }

        ClientUtils.displayChatMessage("§a------------\n§7> §c${FDPNext.commandManager.prefix}help §8<§7§lpage§8>")
    }
}
