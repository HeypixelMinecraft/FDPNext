...
class Spammer : Module(name = "Spammer", category = ModuleCategory.MISC) {
    ...
    fun onUpdate(event: UpdateEvent) {
        ...
        if (modeValue.equals("Single") && messageValue.get().startsWith(".")) {
            FDPNext.commandManager.executeCommands(messageValue.get())
            return
        }
    ...
    private fun replace(str: String): String {
        return str.replace("%r", RandomUtils.nextInt(0, 99).toString())
                    ...
                    .replace("%name%", if (FDPNext.combatManager.target != null) { FDPNext.combatManager.target!!.name } else { "You" }) + (RandomUtils.randomString(endingCharsValue.get().toInt()).toString())
    }
}
