...
class AACPushVelocity : VelocityMode("AACPush") {
    ...
    override fun onVelocity(event: UpdateEvent) {
        if (jump) {
            ...
            if (mc.thePlayer.hurtResistantTime > 0 && aacPushYReducerValue.get() &&
                !FDPNext.moduleManager[Speed::class.java]!!.state) {
    ...
    override fun onJump(event: JumpEvent) {
        ...
        if ((velocity.onlyGroundValue.get() && !mc.thePlayer.onGround) || (velocity.onlyCombatValue.get() && !FDPNext.combatManager.inCombat)) {
            return
        }
