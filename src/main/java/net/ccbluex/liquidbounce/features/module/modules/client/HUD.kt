...
object HUD : Module("HUD", category = ModuleCategory.CLIENT, array = false, defaultOn = true) {
    ...
    fun onRender2D(event: Render2DEvent) {
        ...
        FDPNext.hud.render(false, event.partialTicks)
    ...
    fun onUpdate(event: UpdateEvent) {
        ...
        if (mc.currentScreen == null && lastFontEpsilon != fontEpsilonValue.get()) {
            ...
            alert("You need to reload FDPNext to apply changes!")
        }
    }
    ...
    fun onScreen(event: ScreenEvent) {
        ...
        if (state && blurValue.get() && !mc.entityRenderer.isShaderActive && event.guiScreen != null && !(event.guiScreen is GuiChat || event.guiScreen is GuiHudDesigner)) {
            mc.entityRenderer.loadShader(ResourceLocation("FDPNext/blur.json"))
        } else if (mc.entityRenderer.shaderGroup != null && mc.entityRenderer.shaderGroup!!.shaderGroupName.contains("FDPNext/blur.json")) {
    ...
    fun onKey(event: KeyEvent) {
        FDPNext.hud.handleKey('a', event.key)
    }
