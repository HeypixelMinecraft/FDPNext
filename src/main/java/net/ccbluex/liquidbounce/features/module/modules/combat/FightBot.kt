...
class FightBot : Module(name = "FightBot", category = ModuleCategory.COMBAT) {
    ...
    override fun onEnable() {
        if (!autoJumpValue.get()) FDPNext.moduleManager[Step::class.java]!!.state = true
        if (!autoJumpValue.get()) FDPNext.moduleManager[Step::class.java]!!.modeValue.set("Jump")
    ...
    override fun onDisable() {
        if (!autoJumpValue.get()) FDPNext.moduleManager[Step::class.java]!!.state = false
        thread?.stop()
        backThread?.stop()
