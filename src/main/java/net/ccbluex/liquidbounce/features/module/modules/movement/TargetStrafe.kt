...
class TargetStrafe : Module(name = "TargetStrafe",  category = ModuleCategory.MOVEMENT) {
    ...
    fun onRender3D(event: Render3DEvent) {
        ...
        private fun canStrafe(target: EntityLivingBase?): Boolean {
            return target != null && (!holdSpaceValue.get() || mc.thePlayer.movementInput.jump) && (!onlySpeedValue.get() || FDPNext.moduleManager[Speed::class.java]!!.state)
        }
    ...
    fun toggleStrafe(): Boolean {
        return targetEntity != null && (!holdSpaceValue.get() || mc.thePlayer.movementInput.jump) && (!onlySpeedValue.get() || FDPNext.moduleManager[Speed::class.java]!!.state)
    }
