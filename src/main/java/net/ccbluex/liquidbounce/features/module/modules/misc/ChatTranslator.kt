...
class ChatTranslator : Module(name = "ChatTranslator", category = ModuleCategory.MISC) {
    ...
    private fun getLink(msg: String): String {
        ...
        return when (apiValue.get().lowercase()) {
            "google" -> "http://translate.google.com/translate_a/single?client=gtx&dt=t&dj=1&ie=UTF-8&sl=auto&tl=" + (if (languageValue.equals("chinese")) "zh_cn" else "en_us") + "&q=$message"
