        ...
        when (clipMode.get().lowercase()) {
            ...
            "clip2" -> {
               ...
               if (clipTimes == 2) {
                    if (!clipSmart.get()) {
                        if (!hasWarned) {
                            FDPNext.hud.addNotification(Notification("Clip success", "To successfully clip disable fly now", NotifyType.SUCCESS, 3000))
                            hasWarned = true
                        }
                    } else {
                        if (timer2.hasTimePassed(350)) {
                            shouldClip = false
                            FDPNext.hud.addNotification(Notification("Smart Clip", "Smart Clip stopped cliping, you can disable fly now.", NotifyType.WARNING, 5000))
                            FDPNext.hud.addNotification(Notification("Smart Clip", "If you have tped back, disable Smart Clip or try again.", NotifyType.WARNING, 5000))
                            try {
                                disableLogger = true
                    ...
                    if (!clipSmart.get()) {
                        FDPNext.hud.addNotification(Notification("Clip fail", "Clipped too many times, disable fly and try again", NotifyType.ERROR, 3000))
                    }
                }
