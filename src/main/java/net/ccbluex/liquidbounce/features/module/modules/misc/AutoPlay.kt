...
class AutoPlay : Module(name = "AutoPlay", category = ModuleCategory.MISC) {
            ...
            when (modeValue.get().lowercase()) {
                "supercraft" -> {
                    ...
                    if (text.contains("El juego ya fue iniciado.", true)) {
                        FDPNext.hud.addNotification(Notification(this.name, "Failed to join, retrying...", NotifyType.ERROR, 1755))
                        queueAutoPlay {
                            mc.thePlayer.sendChatMessage("/sw leave")
                            mc.thePlayer.sendChatMessage("/sw randomjoin solo")
                        }
                ...
                "blocksmc" -> {
                    if (clickState == 1 && text.contains("Only VIP players can join full servers!", true)) {
                        FDPNext.hud.addNotification(Notification(this.name, "Join failed! trying again...", NotifyType.WARNING, 3000))
    ...
    private fun queueAutoPlay(delay: Long = delayValue.get().toLong() * 1000, runnable: () -> Unit) {
        ...
        if (this.state) {
            ...
            FDPNext.hud.addNotification(Notification(this.name, "Sending you to next game in ${delayValue.get()}s...", NotifyType.INFO, delayValue.get() * 1000))
        }
    }
