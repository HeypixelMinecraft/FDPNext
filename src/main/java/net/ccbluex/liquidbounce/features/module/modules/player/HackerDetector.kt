...
//        }
    ...
    private fun HackerData.flag(type: String, vl: Int, msg: String) {
        ...
        if (this.vl> vlValue.get()) {
            ...
            if (notifyValue.get()) {
                FDPNext.hud.addNotification(Notification(name, "${this.player.name} might use hack ($use)", NotifyType.WARNING))
            }
            this.vl = -vlValue.get()

            if (reportValue.get()) {
                FDPNext.moduleManager[AutoReport::class.java]!!.doReport(this.player)
            }
        }
    }
