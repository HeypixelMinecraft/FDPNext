...
open class Module @JvmOverloads constructor(
    ...
    set(keyBind) {
        ...
        if (!FDPNext.isStarting) {
            FDPNext.configManager.smartSave()
        }
    }
    var tArray = array
        set(array) {
        ...
        set(value) {
            ...
            if (!FDPNext.isStarting) {
                if (value) {
                    ...
                    FDPNext.hud.addNotification(Notification(LanguageManager.getAndFormat("notify.module.title"), LanguageManager.getAndFormat("notify.module.enable", localizedName), NotifyType.SUCCESS))
                } else {
                    SoundModule.playSound(false)
                    FDPNext.hud.addNotification(Notification(LanguageManager.getAndFormat("notify.module.title"), LanguageManager.getAndFormat("notify.module.disable", localizedName), NotifyType.ERROR))
                }
            }
