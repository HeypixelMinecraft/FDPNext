...
class NameTags : Module(name = "NameTags", category = ModuleCategory.RENDER) {
    ...
    private fun renderNameTag(entity: EntityLivingBase, tag: String) {
        if (onlyTarget.get() && entity != FDPNext.combatManager.target && entity.getName() != entityKeep) {
            return
        } else if (onlyTarget.get() && entity == FDPNext.combatManager.target) {
            entityKeep = entity.getName()
            targetTicks++
            if (targetTicks >= 5) {
                targetTicks = 4
            }
        } else if (onlyTarget.get() && FDPNext.combatManager.target == null) {
            targetTicks--
            if (targetTicks <= -1) {
                targetTicks = 0
                entityKeep = "zywl gg"
