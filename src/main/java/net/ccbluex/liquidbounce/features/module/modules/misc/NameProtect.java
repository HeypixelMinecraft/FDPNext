...
public class NameProtect extends Module {
    ...
    public void onText(final TextEvent event) {
        if (mc.thePlayer == null || Objects.requireNonNull(event.getText()).contains("§8[§9§l" + FDPNext.CLIENT_NAME + "§8] §3") || event.getText().startsWith("/") || event.getText().startsWith(FDPNext.commandManager.getPrefix() + ""))
            return;

        for (final FriendsConfig.Friend friend : FDPNext.fileManager.getFriendsConfig().getFriends())
