...
object Fucker : Module(name = "Fucker", category = ModuleCategory.WORLD) {
    ...
    fun onUpdate(event: UpdateEvent) {
        ...
        when {
            ...
            actionValue.equals("destroy") || surroundings || !isRealBlock -> {
                // Auto Tool
                val autoTool = FDPNext.moduleManager[AutoTool::class.java]!!
                if (autoTool.state) {
    ...
    if (ignoreFirstBlockValue.get() && nearestBlock != null) {
        if (firstPos == null) {
            ...
            FDPNext.hud.addNotification(Notification(name,"Found first ${getBlockName(targetID)} block at ${nearestBlock!!.x.toInt()} ${nearestBlock!!.y.toInt()} ${nearestBlock!!.z.toInt()}",  NotifyType.SUCCESS))
        }
        if (targetID == 26 && firstPos != null && firstPosBed == null) { // bed
            ...
            if (firstPosBed != null)
                FDPNext.hud.addNotification(Notification(name,"Found second Bed block at ${firstPosBed!!.x.toInt()} ${firstPosBed!!.y.toInt()} ${firstPosBed!!.z.toInt()}", NotifyType.SUCCESS))
        }
    }
