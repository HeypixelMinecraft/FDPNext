        ...
        synchronized(particles) {
            for(entity in mc.theWorld.loadedEntityList) {
                if(entity is EntityLivingBase && EntityUtils.isSelected(entity,true)) {
                    ...
                    val colourPrefix = if (colourValue.get().equals("Damage")) (if(lastHealth>entity.health){"Â§c"}else{"Â§a"}) else ""
                    val prefix = if (!colourValue.get().equals("Rainbow")) (if(lastHealth>entity.health){"â?}else{"â?}) else (if(lastHealth>entity.health){"-"}else{"+"})
