...
object GuiCapeManager : GuiScreen() {
    private val jsonFile = File(FDPNext.fileManager.capesDir, "cape.json")
    ...
    init {
        arrayOf("classic", "classic2", "aurora", "forest", "rose", "lavender", "ocean", "modern1", "modern2", "lava", "citrus", "fire", "nightlife", "abstract", "blur", "owner").forEach {
            try {
                embeddedCapes.add(loadCapeFromResource(it, "assets/minecraft/FDPNext/cape/$it.png"))
            } catch (e: Throwable){
    ...
    fun load() {
        ...
        for (file in FDPNext.fileManager.capesDir.listFiles()) {
