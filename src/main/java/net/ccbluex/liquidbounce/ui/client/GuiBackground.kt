...
class GuiBackground(private val prevGui: GuiScreen) : GuiScreen() {
    ...
    private fun updateButtons() {
        ...
        val hasCustomBackground = FDPNext.background != null
    ...
    override fun actionPerformed(button: GuiButton) {
        when (button.id) {
            ...
            3 -> {
                ...
                try {
                    Files.copy(file.toPath(), FileOutputStream(FDPNext.fileManager.backgroundFile))

                    val image = ImageIO.read(FileInputStream(FDPNext.fileManager.backgroundFile))
                    FDPNext.background = ResourceLocation(FDPNext.CLIENT_NAME.lowercase() + "/background.png")
                    mc.textureManager.loadTexture(FDPNext.background, DynamicTexture(image))
                } catch (e: Exception) {
                    e.printStackTrace()
                    MiscUtils.showErrorPopup("Error", "Exception class: " + e.javaClass.name + "\nMessage: " + e.message)
                    FDPNext.fileManager.backgroundFile.delete()
                }
            }
            4 -> {
                FDPNext.background = null
                FDPNext.fileManager.backgroundFile.delete()
            }
            5 -> {
    ...
    override fun drawScreen(mouseX: Int, mouseY: Int, partialTicks: Float) {
        ...
        mc.fontRendererObj.drawCenteredString("%ui.background.gcurrent%: " + if(FDPNext.background == null) { GradientBackground.nowGradient.name } else { "Customized" },
