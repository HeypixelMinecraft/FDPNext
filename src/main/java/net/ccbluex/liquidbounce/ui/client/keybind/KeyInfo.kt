...
class KeyInfo(
    ...
    fun update() {
        modules = FDPNext.moduleManager.getKeyBind(key) as ArrayList<Module>
        macros = FDPNext.macroManager.macros.filter { it.key == key } as ArrayList<Macro>
    ...
    fun click(mouseX: Float, mouseY: Float) {
        val keyBindMgr = FDPNext.keyBindManager
        ...
        if (keyBindMgr.nowDisplayKey == null) {
            ...
            if (scaledMouseY> 22F + Fonts.font40.height &&
                ...
                if (scaledMouseY> baseTabHeight - 22F - Fonts.font40.height) {
                    ...
                    for (macro in macros) {
                        if (scaledMouseY> (yOffset + 5) && scaledMouseY <(yOffset + 15)) {
                            FDPNext.macroManager.macros.remove(macro)
                            update()
                            break
                        }
                        yOffset += 20
