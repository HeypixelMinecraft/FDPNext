...
class GuiHudDesigner : GuiScreen() {
    ...
    override fun drawScreen(mouseX: Int, mouseY: Int, partialTicks: Float) {
        FDPNext.hud.render(true, partialTicks)
        FDPNext.hud.handleMouseMove(mouseX, mouseY)

        if (!FDPNext.hud.elements.contains(selectedElement))
        ...
        if (wheel != 0) {
            for (element in FDPNext.hud.elements) {
    ...
    override fun mouseClicked(mouseX: Int, mouseY: Int, mouseButton: Int) {
        ...
        FDPNext.hud.handleMouseClick(mouseX, mouseY, mouseButton)
    ...
    override fun mouseReleased(mouseX: Int, mouseY: Int, state: Int) {
        ...
        FDPNext.hud.handleMouseReleased()
    }

    override fun onGuiClosed() {
        Keyboard.enableRepeatEvents(false)
        FDPNext.fileManager.saveConfig(FDPNext.fileManager.hudConfig)
    ...
    override fun keyTyped(typedChar: Char, keyCode: Int) {
        when (keyCode) {
            Keyboard.KEY_DELETE ->
                if (selectedElement != null) FDPNext.hud.removeElement(selectedElement!!)
            ...
            else -> FDPNext.hud.handleKey(typedChar, keyCode)
        }
