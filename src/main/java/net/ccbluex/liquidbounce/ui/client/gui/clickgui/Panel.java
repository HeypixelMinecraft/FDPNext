...
public abstract class Panel extends MinecraftInstance {
    ...
    public void drawScreen(int mouseX, int mouseY, float button) {
        ...
        final int maxElements = FDPNext.moduleManager.getModule(ClickGUIModule.class).getMaxElementsValue().get();
    ...
    private int getElementsHeight() {
        ...
        int count = 0;
        for(final Element element : elements) {
            if (count >= FDPNext.moduleManager.getModule(ClickGUIModule.class).getMaxElementsValue().get())
                continue;
