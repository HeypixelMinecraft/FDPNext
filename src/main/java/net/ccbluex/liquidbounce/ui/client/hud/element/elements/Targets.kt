...
open class Targets : Element(-46.0, -40.0, 1F, Side(Side.Horizontal.MIDDLE, Side.Vertical.MIDDLE)) {
    ...
    private fun drawAstolfo(target: EntityLivingBase) {
        ...
        font.drawString("${getHealth(target).roundToInt()} HP", 19, 9, color.rgb)
        GL11.glPopMatrix()
        if (shadowValue.get()) {
    ...
    private fun drawAstolfo2(entity: EntityPlayer) {
        ...
        GL11.glScalef(2F, 2F, 2F)
        Fonts.minecraftFont.drawString("${decimalFormat3.format(entity.health)} HP", 0, 0, barColor.rgb)
        GL11.glPopMatrix()
    ...
    private fun drawZamorozka(target: EntityLivingBase) {
        ...
        font.drawCenteredString("${target.name} ${if (target.ping != -1) { "搂f${target.ping}ms" } else { "" }}", 93f, 16f - font.FONT_HEIGHT, ColorUtils.rainbow().rgb, false)
        font.drawString("Health: ${decimalFormat.format(easingHP)} 搂7/ ${decimalFormat.format(target.maxHealth)}", 43, 11 + font.FONT_HEIGHT, Color.WHITE.rgb)
    ...
    private fun drawRise(target: EntityLivingBase) {
        ...
        // 鍙椾激鐨勭缉鏀炬晥鏋?        GL11.glScalef(scale, scale, scale)
        GL11.glTranslatef(((size * 0.5f * (1 - scale)) / scale), ((size * 0.5f * (1 - scale)) / scale), 0f)
        // 鍙椾激鐨勭孩鑹叉晥鏋?        GL11.glColor4f(1f, 1 - hurtPercent, 1 - hurtPercent, 1f)
        // 缁樺埗澶撮儴鍥剧墖
...
/*
    ...
    private fun drawSlowly(entity: EntityLivingBase) {
        val font = Fonts.minecraftFont
        val healthString = "${decimalFormat2.format(entity.health)} HP"
