...
public class BjurStyle extends GuiScreen {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        if (isCategoryHovered(startX + 100.0F, startY + 40.0F, startX + 200.0F, startY + 315.0F, mouseX, mouseY)) {
            if (dWheel < 0 && moduleStart < FDPNext.moduleManager.getModuleInCategory(currentModuleType).size() - 1)
                moduleStart++;
        ...
        for (int i = 0; i < FDPNext.moduleManager.getModuleInCategory(currentModuleType).size(); i++) {
            Module module = FDPNext.moduleManager.getModuleInCategory(currentModuleType).get(i);
            if (mY > startY + 250.0F)
                break;
            if (i >= moduleStart) {
                if (!module.getState()) {
        ...
        for (int i2 = 0; i2 < (ModuleCategory.values()).length; i2++) {
            ...
            try {
                if (isCategoryHovered(xx2 + 8.0F, k2 - 10.0F + 60.0F + (i2 * 45), xx2 + 80.0F, k2 + 20.0F + 60.0F + (i2 * 45), mouseX, mouseY) && Mouse.isButtonDown(0)) {
                    ...
                    currentModule = FDPNext.moduleManager.getModuleInCategory(currentModuleType).get(0);
                    moduleStart = 0;
                    valueStart = 0;
    ...
    public void keyTyped(char typedChar, int keyCode) {
        if (this.bind) {
            ...
            ((ClickGUIModule) FDPNext.moduleManager.getModule(ClickGUIModule.class)).setState(false);
    ...
    public static Module currentModule = FDPNext.moduleManager.getModuleInCategory(currentModuleType).get(0);
