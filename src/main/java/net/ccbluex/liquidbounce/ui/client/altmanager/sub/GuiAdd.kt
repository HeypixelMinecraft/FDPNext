...
class GuiAdd(private val prevGui: GuiAltManager) : GuiScreen() {
    ...
    override fun actionPerformed(button: GuiButton) {
        ...
        when (button.id) {
            ...
            1 -> {
                if (FDPNext.fileManager.accountsConfig.altManagerMinecraftAccounts.any { it.name == username.text }) {
                    status = "§c${LanguageManager.getAndFormat("ui.alt.alreadyAdded")}"
                    return
                }
                FDPNext.fileManager.accountsConfig.altManagerMinecraftAccounts.add(AccountSerializer.accountInstance(username.text, password.text))
