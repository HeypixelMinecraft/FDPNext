...
public class ClickGui extends GuiScreen {
        ...
        for (final ModuleCategory category : ModuleCategory.values()) {
            panels.add(new Panel(category.getDisplayName(), category, 100, yPos, width, height, false) {
                ...
                public void setupItems() {
                    for (Module module : FDPNext.moduleManager.getModules())
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        float trueCguiScale;
        if (FDPNext.moduleManager.getModule(ClickGUIModule.class).INSTANCE.getStyleValue().get().equals("Jello")) {
            trueCguiScale = 1;
        } else if(FDPNext.moduleManager.getModule(ClickGUIModule.class).INSTANCE.getStyleValue().get().equals("Glow")) {
            trueCguiScale = 1;
        } else {
            trueCguiScale = FDPNext.moduleManager.getModule(ClickGUIModule.class).INSTANCE.getScaleValue().get();
        }
        final double scale = trueCguiScale;
        if (progress < 1) progress += 0.1 * (1 - partialTicks);
        else progress = 1;

        switch (Objects.requireNonNull(FDPNext.moduleManager.getModule(ClickGUIModule.class)).INSTANCE.getAnimationValue().get().toLowerCase()) {
            case "liquidbounce":
            case "ziul":
        ...
        switch (Objects.requireNonNull(FDPNext.moduleManager.getModule(ClickGUIModule.class)).INSTANCE.getBackgroundValue().get()) {
            case "Default":
        ...
        switch (Objects.requireNonNull(FDPNext.moduleManager.getModule(ClickGUIModule.class)).getAnimationValue().get().toLowerCase()) {
            case "bread":
            case "slide":
            case "liquidbounce":
                GlStateManager.translate(0, (1.0 - slide) * height * -2.0, 0);
    ...
    protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
        float trueCguiScale;
        if (FDPNext.moduleManager.getModule(ClickGUIModule.class).getStyleValue().get().equals("Jello")) {
            trueCguiScale = 1;
        } else if(FDPNext.moduleManager.getModule(ClickGUIModule.class).getStyleValue().get().equals("Glow")) {
            trueCguiScale = 1;
        } else {
            trueCguiScale = FDPNext.moduleManager.getModule(ClickGUIModule.class).getScaleValue().get();
        }
        final double scale = trueCguiScale;
    ...
    public void onGuiClosed() {
        FDPNext.fileManager.saveConfig(modernuiLaunchOption.getClickGuiConfig());
        slide = 0;
	progress = 0;
    }
