    ...
    override fun drawElement(partialTicks: Float): Border? {
        ...
        FDPNext.hud.notifications.map { it }.forEachIndexed { index, notify ->
            ...
            if (notify.drawNotification(index, FontLoaders.C16, backGroundAlphaValue.get(), blurValue.get(), this.renderX.toFloat(), this.renderY.toFloat(), scale,contentShadow.get(),titleShadow.get(),motionBlur.get(),whiteText.get(),modeColored.get(), Companion)) {
                FDPNext.hud.notifications.remove(notify)
            }
        ...
        if (mc.currentScreen is GuiHudDesigner) {
            if (!FDPNext.hud.notifications.contains(exampleNotification)) {
                FDPNext.hud.addNotification(exampleNotification)
            }
...
class Notification(
    ...
    fun drawNotification(
        ...
        val error = ResourceLocation("FDPNext/ui/notifications/icons/tenacity/cross.png")
        val successful = ResourceLocation("FDPNext/ui/notifications/icons/tenacity/tick.png")
        val warn = ResourceLocation("FDPNext/ui/notifications/icons/tenacity/warning.png")
        val info = ResourceLocation("FDPNext/ui/notifications/icons/tenacity/info.png")
        ...
        if(style.equals("Intellij")) {
                val notifyDir = "FDPNext/notifications/icons/noti/intellij/"
