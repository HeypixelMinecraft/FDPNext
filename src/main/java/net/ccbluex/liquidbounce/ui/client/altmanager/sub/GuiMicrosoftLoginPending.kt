    ...
    override fun initGui() {
        server = MicrosoftAccount.Companion.buildFromOpenBrowser(object : MicrosoftAccount.OAuthHandler {
            ...
            override fun authResult(account: MicrosoftAccount) {
                if (FDPNext.fileManager.accountsConfig.altManagerMinecraftAccounts.any { it.name == account.name }) {
                    stage = "§c${LanguageManager.getAndFormat("ui.alt.alreadyAdded")}"
                    return
                }
                FDPNext.fileManager.accountsConfig.altManagerMinecraftAccounts.add(account)
