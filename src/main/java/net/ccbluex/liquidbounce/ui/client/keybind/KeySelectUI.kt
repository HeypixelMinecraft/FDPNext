...
class KeySelectUI(val info: KeyInfo) : PopUI(LanguageManager.get("ui.keybind.select")) {
    private var str = ""
    private var modules = FDPNext.moduleManager.modules.toList()
    ...
    override fun key(typedChar: Char, keyCode: Int) {
        if (keyCode == Keyboard.KEY_BACK) {
            ...
            if (str.startsWith(".")) {
                FDPNext.macroManager.macros.add(Macro(info.key, str))
                FDPNext.keyBindManager.updateAllKeys()
                close()
    ...
    override fun close() {
        FDPNext.keyBindManager.popUI = null
    }

    private fun update() {
        modules = if (str.isNotEmpty()) {
            FDPNext.moduleManager.modules.filter { it.name.startsWith(str, ignoreCase = true) }
        } else {
            FDPNext.moduleManager.modules.toList()
        }
