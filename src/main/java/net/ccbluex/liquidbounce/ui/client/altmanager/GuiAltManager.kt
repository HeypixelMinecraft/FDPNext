...
class GuiAltManager(private val prevGui: GuiScreen) : GuiScreen() {
    ...
    override fun drawScreen(mouseX: Int, mouseY: Int, partialTicks: Float) {
        ...
        mc.fontRendererObj.drawCenteredString(LanguageManager.getAndFormat("ui.alt.alts", FDPNext.fileManager.accountsConfig.altManagerMinecraftAccounts.size), (width / 2).toFloat(), 18f, 0xffffff)
    ...
    override fun actionPerformed(button: GuiButton) {
        ...
        when (button.id) {
            ...
            2 -> status = if (altsList.selectedSlot != -1 && altsList.selectedSlot < altsList.size) {
                FDPNext.fileManager.accountsConfig.altManagerMinecraftAccounts.removeAt(altsList.selectedSlot)
                FDPNext.fileManager.saveConfig(FDPNext.fileManager.accountsConfig)
            ...
            3 -> if (altsList.selectedSlot != -1 && altsList.selectedSlot < altsList.size) {
                Thread {
                    val minecraftAccount = FDPNext.fileManager.accountsConfig.altManagerMinecraftAccounts[altsList.selectedSlot]
            ...
            4 -> {
                if (FDPNext.fileManager.accountsConfig.altManagerMinecraftAccounts.size <= 0) {
                    status = "§c${LanguageManager.getAndFormat("ui.alt.emptyList")}"
                    return
                }
                val randomInteger = Random().nextInt(FDPNext.fileManager.accountsConfig.altManagerMinecraftAccounts.size)
                if (randomInteger < altsList.size) altsList.selectedSlot = randomInteger
                Thread {
                    val minecraftAccount =
                        FDPNext.fileManager.accountsConfig.altManagerMinecraftAccounts[randomInteger]
                    status = "§a${LanguageManager.getAndFormat("ui.alt.loggingIn")}"
                    status = login(minecraftAccount)
                }.start()
            }
    ...
    private inner class GuiList(prevGui: GuiScreen)
        ...
        var selectedSlot = 0
            get() {
                if (field > FDPNext.fileManager.accountsConfig.altManagerMinecraftAccounts.size)
                    field = -1
                return field
            }
        ...
        public override fun getSize(): Int {
            return FDPNext.fileManager.accountsConfig.altManagerMinecraftAccounts.size
        }
        ...
        override fun drawSlot(id: Int, x: Int, y: Int, var4: Int, var5: Int, var6: Int) {
            val minecraftAccount = FDPNext.fileManager.accountsConfig.altManagerMinecraftAccounts[id]
    ...
    companion object {
        ...
        fun login(account: MinecraftAccount): String {
            return try {
                ...
                FDPNext.eventManager.callEvent(SessionEvent())
