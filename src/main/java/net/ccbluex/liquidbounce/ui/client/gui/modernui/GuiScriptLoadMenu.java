    ...
    protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
        ...
        if (menuType == ScriptMenuType.Local) {
            if (mouseX > x + 450 && mouseX < x + 490 && mouseY > y + 280 && mouseY < y + 295) {
                ...
                FDPNext.hud.addNotification(new Notification("Script Manager", "Reloading Scripts..", NotifyType.INFO, 1500, 500));
                ...
                FDPNext.hud.addNotification(new Notification("Script Manager", "Reload Successful (" + (System.currentTimeMillis() - startTime) + "ms)", NotifyType.SUCCESS, 1500, 500));
            }
            ...
            try {
                ...
                FDPNext.scriptManager.getScripts().stream().filter(script -> !script.isOnline()).forEach(scriptList::add);
                for (Script script : scriptList) {
                    if (x + 450 < mouseX && y + 41 - scroll + (i * 30) < mouseY && mouseX < x + 490 && mouseY < y - scroll + 59 + (i * 30)) {
                        if (script.getState()) {
                            FDPNext.hud.addNotification(new Notification("Script Manager", "Unload " + script.scriptName, NotifyType.INFO, 1500, 500));
                            for (Module registeredModule : script.getRegisteredModules()) {
                                registeredModule.setState(false);
                            }
                            script.onDisable();
                        } else {
                            FDPNext.hud.addNotification(new Notification("Script Manager", "Load " + script.scriptName, NotifyType.INFO, 1500, 500));
                            script.onEnable();
                            script.regAnyThing();
                        }
                    }
            ...
            if (mouseX > x + 405 && mouseX < x + 445 && mouseY > y + 280 && mouseY < y + 295) {
                FDPNext.hud.addNotification(new Notification("Script Manager", "Unload Scripts", NotifyType.INFO, 1500, 500));
                FDPNext.scriptManager.disableScripts();
                FDPNext.scriptManager.unloadScripts();
            }
            int i = 0;
            try {
                List<Script> scriptList = new ArrayList<>();
                FDPNext.scriptManager.getScripts().stream().filter(Script::isOnline).forEach(scriptList::add);
                for (Script script : scriptList) {
                    if (x + 450 < mouseX && y + 41 - scroll + (i * 30) < mouseY && mouseX < x + 490 && mouseY < y - scroll + 59 + (i * 30)) {
                        if (script.getState()) {
                            FDPNext.hud.addNotification(new Notification("Script Manager", "Unload " + script.scriptName, NotifyType.INFO, 1500, 500));
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        if (isClickSub) {
            try {
                ...
                FDPNext.scriptManager.loadScripts();
                FDPNext.scriptManager.enableScripts();
                FDPNext.hud.addNotification(new Notification("Script Manager", "Added Subscribe: " + name + " | " + url + " (" + (System.currentTimeMillis() - startTime) + "ms)", NotifyType.SUCCESS, 1500, 500));
                isClickSub = false;
            } catch (Exception e) {
                e.printStackTrace();
            }
        ...
        CFontRenderer.DisplayFonts(FontLoaders.F40, "FDPNext", x + 10, y + 10, new Color(255, 255, 255, 255).getRGB());
        if (menuType == ScriptMenuType.Local) {
            ...
            // 热交换舒服，所以就不创建在RendererUtils创建方法�?            CFontRenderer.DisplayFonts(FontLoaders.F30, "Script Manager", x + 135, y + 35, new Color(255, 255, 255, 255).getRGB());
            ...
            CFontRenderer.DisplayFonts(FontLoaders.C16, "Loaded " + FDPNext.scriptManager.getScripts().size() + " Local scripts", x + 145, y + 85, new Color(217, 217, 217).getRGB());
            ...
            FDPNext.scriptManager.getScripts().forEach(script -> is.addAndGet(script.getRegisteredModules().size()));
            CFontRenderer.DisplayFonts(FontLoaders.C16, "Loaded " + is + " module from " + FDPNext.scriptManager.getScripts().size() + " scripts", x + 145, y + 105, new Color(217, 217, 217).getRGB());
            AtomicInteger i1 = new AtomicInteger();
            FDPNext.scriptManager.getScripts().forEach(script -> i1.addAndGet((int) script.getRegisteredModules().stream().filter(Module::getState).count()));
            CFontRenderer.DisplayFonts(FontLoaders.C16, "Enable " + i1 + " module from " + FDPNext.scriptManager.getScripts().size() + " scripts", x + 145, y + 115, new Color(217, 217, 217).getRGB());
            CFontRenderer.DisplayFonts(FontLoaders.C14, "Tips: FDPNext is not responsible for script security, if you have any doubts, please consult the developer", x + 130, y + 140, new Color(161, 161, 161).getRGB());
